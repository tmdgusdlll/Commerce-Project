...
public class Storage {
     ...
     * CommerceSystem 클래스는 Product 클래스, Category 클래스 몰라도 됨. Storage 클래스에서 카테고리를 그냥 요청하면 됨.
    ...
    // 생성자
    public Storage() {
        // 객체 생성 시 완전한 상태의 각 카테고리 별 상품 리스트(전자제품, 의류, 식품)를 보여줄 수 있다.
        productList();

    }

    public void productList() {
        elecProducts.add(new Product("Galaxy S24", 1200000, "최신 안드로이드 스마트폰", 50));
        elecProducts.add(new Product("iphone 15", 1350000, "Apple의 최신 스마트폰", 30));
        elecProducts.add(new Product("MacBook Pro", 2400000, "M3 칩셋이 탑재된 노트북", 20));
        elecProducts.add(new Product("AirPods Pro", 350000, "노이즈 캔슬링 무선 이어폰", 70));

        clothProducts.add(new Product("오프화이트 후드티", 129000, "오프화이트 시그니처 로고 후드티", 40));
        clothProducts.add(new Product("무신사 스탠다드 데님 팬츠", 69900, "슬림 스트레이트 핏 데님 팬츠", 120));
        clothProducts.add(new Product("노스페이스 눕시 패딩", 359000, "보온성 뛰어난 겨울용 다운 패딩", 25));
        clothProducts.add(new Product("아디다스 트랙 자켓", 99000, "데일리 착용 가능한 트랙 자켓", 70));

        foodProducts.add(new Product("한우 등심 1++ (300g)", 59800, "국내산 1++ 등급 한우 등심", 35));
        foodProducts.add(new Product("제주 삼다수 2L x 6", 6900, "제주 화산암반수 생수", 200));
        foodProducts.add(new Product("신라면 멀티팩 (5입)", 4200, "농심 대표 매운 라면", 500));
        foodProducts.add(new Product("동원 참치 라이트스탠다드", 2300, "담백한 맛의 캔 참치", 180));
    }

    // 카테고리 객체 생성
    public List<Category> createCategories() {
        List<Category> categories = new ArrayList<>();
        categories.add(new Category("전자제품" ,elecProducts));
        categories.add(new Category("의류" ,clothProducts));
        categories.add(new Category("음식" ,foodProducts));
        return categories;
    }
...
//        return new ArrayList<>(List.of(elec, cloth, food));
//    }
//
//
...
//    ));
//    private List<Product> clothProducts = new ArrayList<>(List.of(
...
//    ));
//    private List<Product> foodProducts = new ArrayList<>(List.of(
